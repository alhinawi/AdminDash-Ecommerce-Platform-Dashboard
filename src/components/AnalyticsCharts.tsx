...
const AnalyticsCharts = ({ products }: AnalyticsChartsProps) => {
  ...
  return (
    <div className="mb-10 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
      {/* Header & Tabs */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-y-4 pb-6 border-b border-gray-100">
        <div>
          <div className="flex items-center gap-x-2">
            <span className="flex h-2.5 w-2.5 rounded-full bg-indigo-600" />
            <h3 className="text-lg font-bold text-gray-900">Catalog Analytics & Insights</h3>
          </div>
        ...
        {/* Tab Controls */}
        <div className="flex items-center rounded-xl bg-gray-100/80 p-1 text-xs font-medium">
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-6">
        ...
        <div className={`lg:col-span-2 space-y-4 ${activeTab === "pricing" ? "hidden lg:block" : ""}`}>
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider text-xs">
            ...
            {categoryList.map((cat) => (
              <div key={cat.name} className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-x-2 font-medium text-gray-800">
                    {cat.imageURL && (
                      <img
                        src={cat.imageURL}
                        alt={cat.name}
                    ...
                    <span>{cat.name}</span>
                    <span className="text-gray-400 font-normal">({cat.count} items)</span>
                  </div>
        ...
        <div className={`space-y-4 ${activeTab === "categories" ? "hidden lg:block" : ""}`}>
          ...
          <div className="flex h-44 items-end gap-x-3 rounded-xl border border-gray-100 bg-slate-50/50 p-4">
            {priceTiers.map((tier) => {
              const heightPercent =
              ...
              return (
                <div key={tier.label} className="flex flex-1 flex-col items-center gap-y-2 h-full justify-end">
                  <span className="text-[10px] font-bold text-gray-600">{tier.count}</span>
                  <div
                    className={`w-full rounded-t-lg ${tier.color} transition-all duration-500 hover:opacity-90 shadow-xs`}
                    style={{ height: `${heightPercent}%` }}
                  />
                  <span className="text-[9px] font-medium text-gray-500 text-center line-clamp-1">
                    {tier.label.split(" ")[0]}
                  </span>
                </div>
              );
            ...
            <div className="flex flex-wrap items-center gap-2">
              {colorList.map(([color, count]) => {
                ...
                return (
                  <div
                    key={color}
                    className="flex items-center gap-x-1.5 rounded-lg border border-gray-100 bg-white px-2 py-1 shadow-2xs text-[11px]"
                  >
                    <span
